...
<head>
    ...
    <script>
        ...
        const myKey = "<%=doctor_uuid%>";
        const otherKey = "<%=patient_uuid%>";
        ...
        const connectSocket = async () =>{
            ...
            stompClient.connect({}, function () {
                ...
                stompClient.subscribe(`/topic/peer/iceCandidate/<%=doctor_uuid%>`, candidate => {
                ...
                stompClient.subscribe(`/topic/peer/offer/<%=doctor_uuid%>`, offer => {
                    const key = JSON.parse(offer.body).key;
                    const message = JSON.parse(offer.body).body;
                    const rtcSession = new RTCSessionDescription({type : message.type,
                                                                  sdp  : message.sdp});
                    d_pc = createPeerConnection();
                    d_pc.setRemoteDescription(rtcSession);
                    //pcListMap.set(key,createPeerConnection(key));
                    //pcListMap.get(key).setRemoteDescription(rtcSession);
                    sendAnswer(d_pc, key);
                });
            });
        }
        ...
        let sendAnswer = (pc) => {
            pc.createAnswer().then( answer => {
                ...
                stompClient.send('/app/peer/answer/' + otherKey, {}, JSON.stringify({
                    key : myKey,
                    body : answer
                }));
